 ...
 *
 * A primitive FDW implementatiton that also monitors
 * FDW callback flow.
 *
 *-------------------------------------------------------------------------
 */
 ...
 * SQL functions
 */
extern Datum flowlog_fdw_handler(PG_FUNCTION_ARGS);
extern Datum flowlog_fdw_validator(PG_FUNCTION_ARGS);

PG_FUNCTION_INFO_V1(flowlog_fdw_handler);
PG_FUNCTION_INFO_V1(flowlog_fdw_validator);
...
Filika::Logger theLogger("/home/postgres/logsocket");
...
// *************************************************************************
// **               PostreSQL FDW Event Handlers                          **
// *************************************************************************

#if (PG_VERSION_NUM >= 90200)
static void flowlogGetForeignRelSize(   PlannerInfo *root,
                                        RelOptInfo *baserel,
                                        Oid foreigntableid );

static void flowlogGetForeignPaths( PlannerInfo *root,
                                    RelOptInfo *baserel,
                                    Oid foreigntableid );

#if (PG_VERSION_NUM < 90500)
static ForeignScan *flowlogGetForeignPlan(  PlannerInfo *root,
                                            RelOptInfo *baserel,
                                            Oid foreigntableid,
...
#else /* 9.1 only */
static FdwPlan*        flowlogPlanForeignScan(  Oid foreigntableid,
                                                PlannerInfo *root,
                                                RelOptInfo *baserel);
#endif

static void            flowlogBeginForeignScan  (ForeignScanState *node, int eflags);
static TupleTableSlot *flowlogIterateForeignScan(ForeignScanState *node);
static void            flowlogReScanForeignScan (ForeignScanState *node);
static void            flowlogEndForeignScan    (ForeignScanState *node);

#if (PG_VERSION_NUM >= 90300)
static void flowlogAddForeignUpdateTargets(Query *parsetree,
                                           RangeTblEntry *target_rte,
                                           Relation target_relation);

static List *flowlogPlanForeignModify(PlannerInfo *root,
                                      ModifyTable *plan,
                                      Index resultRelation,
                                      int subplan_index);

static void flowlogBeginForeignModify(ModifyTableState *mtstate,
                                      ResultRelInfo *rinfo,
                                      List *fdw_private,
                                      int subplan_index,
                                      int eflags);

static TupleTableSlot *flowlogExecForeignInsert(EState *estate,
                                                ResultRelInfo *rinfo,
                                                TupleTableSlot *slot,
                                                TupleTableSlot *planSlot);

static TupleTableSlot *flowlogExecForeignUpdate(EState *estate,
                                                ResultRelInfo *rinfo,
                                                TupleTableSlot *slot,
                                                TupleTableSlot *planSlot);

static TupleTableSlot *flowlogExecForeignDelete(EState *estate,
                                                ResultRelInfo *rinfo,
                                                TupleTableSlot *slot,
                                                TupleTableSlot *planSlot);

static void flowlogEndForeignModify(EState *estate, ResultRelInfo *rinfo);

static int  flowlogIsForeignRelUpdatable(Relation rel);

#endif

static void flowlogExplainForeignScan(  ForeignScanState *node,
                                        struct ExplainState * es);

#if (PG_VERSION_NUM >= 90300)
static void flowlogExplainForeignModify(ModifyTableState *mtstate,
                                        ResultRelInfo *rinfo,
                                        List *fdw_private,
                                        int subplan_index,
                                        struct ExplainState * es);
...
#if (PG_VERSION_NUM >= 90200)
static bool flowlogAnalyzeForeignTable( Relation relation,
...
static void flowlogGetForeignJoinPaths( PlannerInfo *root,
                                        RelOptInfo *joinrel,
                                        RelOptInfo *outerrel,
                                        RelOptInfo *innerrel,
                                        JoinType jointype,
...
static RowMarkType flowlogGetForeignRowMarkType(RangeTblEntry *rte,
                                                LockClauseStrength strength);

static HeapTuple flowlogRefetchForeignRow(  EState *estate,
                                            ExecRowMark *erm,
                                            Datum rowid,
                                            bool *updated);

static List *flowlogImportForeignSchema(ImportForeignSchemaStmt *stmt,
                                        Oid serverOid);
 ...
 *
 * These next structures are not actually used by flowlog,but something like
...
/*
 * The plan state is set up in flowlogGetForeignRelSize and stashed away in
 * baserel->fdw_private and fetched in flowlogGetForeignPaths.
 */
typedef struct
{
    char       *foo;
 ...
 *
 * It is set up in flowlogBeginForeignScan and stashed in node->fdw_state
 * and subsequently used in flowlogIterateForeignScan,
 * flowlogEndForeignScan and flowlogReScanForeignScan.
 */
typedef struct
{
    int         idx;
 ...
 *
 * It is set up in flowlogBeginForeignModify and stashed in
 * rinfo->ri_FdwState and subsequently used in flowlogExecForeignInsert,
 * flowlogExecForeignUpdate, flowlogExecForeignDelete and
 * flowlogEndForeignModify.
 */
typedef struct
{
    char       *chimp;
    int         chump;
} FlowlogFdwModifyState;



static void PrintOptionList(List *optionList, const char *msg = nullptr)
{
    if (msg) LOG_LINE_STR(theLogger, msg);

    ListCell  *cell;
    foreach(cell, optionList)
    {
        DefElem *def = (DefElem *) lfirst(cell);
        LOG_LINE_STR(theLogger, def->defname ? def->defname : "null");
        char *val = defGetString(def);
        LOG_LINE_STR(theLogger, val ? val : "null");
...
static void fileGetOptions( Oid foreigntableid,
                            ...
                            List **other_options)
{
    ForeignTable        *table   = nullptr;
    ForeignServer       *server  = nullptr;
    ForeignDataWrapper  *wrapper = nullptr;
    List                *options = nullptr;
    ListCell            *lc      = nullptr,
                        *prev    = nullptr;
    ...
    //PrintOptionList(options_list, "fileGetOptions");
...
Datum flowlog_fdw_handler(PG_FUNCTION_ARGS)
{
    LOG_LINE_STR(theLogger, "Ver:0.0.0 Build 1");
    ...
    /* these are required */
#if (PG_VERSION_NUM >= 90200)
    fdwroutine->GetForeignRelSize       = flowlogGetForeignRelSize;             /* S U D */
    fdwroutine->GetForeignPaths         = flowlogGetForeignPaths;               /* S U D */
    fdwroutine->GetForeignPlan          = flowlogGetForeignPlan;                /* S U D */
#else
    fdwroutine->PlanForeignScan         = flowlogPlanForeignScan;               /* S */
#endif
    fdwroutine->BeginForeignScan        = flowlogBeginForeignScan;              /* S U D */
    fdwroutine->IterateForeignScan      = flowlogIterateForeignScan;            /* S */
    fdwroutine->ReScanForeignScan       = flowlogReScanForeignScan;             /* S */
    fdwroutine->EndForeignScan          = flowlogEndForeignScan;                /* S U D */

    /* remainder are optional - use NULL if not required */
    /* support for insert / update / delete */
#if (PG_VERSION_NUM >= 90300)
    fdwroutine->IsForeignRelUpdatable   = flowlogIsForeignRelUpdatable;
    fdwroutine->AddForeignUpdateTargets = flowlogAddForeignUpdateTargets;       /* U D */
    fdwroutine->PlanForeignModify       = flowlogPlanForeignModify;             /* I U D */
    fdwroutine->BeginForeignModify      = flowlogBeginForeignModify;            /* I U D */
    fdwroutine->ExecForeignInsert       = flowlogExecForeignInsert;             /* I */
    fdwroutine->ExecForeignUpdate       = flowlogExecForeignUpdate;             /* U */
    fdwroutine->ExecForeignDelete       = flowlogExecForeignDelete;             /* D */
    fdwroutine->EndForeignModify        = flowlogEndForeignModify;              /* I U D */
#endif

    /* support for EXPLAIN */
    fdwroutine->ExplainForeignScan      = flowlogExplainForeignScan;            /* EXPLAIN S U D */
#if (PG_VERSION_NUM >= 90300)
    fdwroutine->ExplainForeignModify    = flowlogExplainForeignModify;          /* EXPLAIN I U D */
#endif

#if (PG_VERSION_NUM >= 90200)
    /* support for ANALYSE */
    fdwroutine->AnalyzeForeignTable     = flowlogAnalyzeForeignTable;           /* ANALYZE only */
#endif


#if (PG_VERSION_NUM >= 90500)
    /* Support functions for IMPORT FOREIGN SCHEMA */
    fdwroutine->ImportForeignSchema     = flowlogImportForeignSchema;

    /* Support for scanning foreign joins */
    fdwroutine->GetForeignJoinPaths     = flowlogGetForeignJoinPaths;

    /* Support for locking foreign rows */
    fdwroutine->GetForeignRowMarkType   = flowlogGetForeignRowMarkType;
    fdwroutine->RefetchForeignRow       = flowlogRefetchForeignRow;
...
Datum flowlog_fdw_validator(PG_FUNCTION_ARGS)
{
    LOG_LINE_STR(theLogger, "Ver:0.0.0 Build 1");
    ...
    LOG_LINE_STR(theLogger, std::to_string(list_length(options_list)).c_str() );

    PrintOptionList(options_list, "Validator");
    ...
    if (list_length(options_list) > 0)
        ereport(ERROR,
                (errcode(ERRCODE_FDW_INVALID_OPTION_NAME),
                 ...
                 errhint("Flowlog FDW does not support any options")));
    */
    PG_RETURN_VOID();
}
                                        ...
                                        Oid         foreigntableid)
{
     ...
     */
    LOG_LINE_STR (theLogger, "\nBeginning Query");
    LogLine(theLogger, "root->type : %s", text_enum(root->type).c_str());
    ListCell   *ppl;
    ...
    List *options = nullptr;
    fileGetOptions(foreigntableid, nullptr, nullptr, &options);

    PrintOptionList(options, "RelSize");
    ...
    //FlowlogFdwPlanState *plan_state = (FlowlogFdwPlanState *) palloc0(sizeof(FlowlogFdwPlanState));
...
static void
flowlogGetForeignPaths(PlannerInfo *root,
                         RelOptInfo *baserel,
                         Oid foreigntableid)
{
    LOG_LINE(theLogger);
    ...
    /*
     * FlowlogFdwPlanState *plan_state = baserel->fdw_private;
     */
...
#if (PG_VERSION_NUM < 90500)
static ForeignScan *
flowlogGetForeignPlan(PlannerInfo *root,
                        RelOptInfo *baserel,
                        Oid foreigntableid,
                        ForeignPath *best_path,
                        List *tlist,
...
static FdwPlan *
flowlogPlanForeignScan(Oid foreigntableid, PlannerInfo *root, RelOptInfo *baserel)
{
    LOG_LINE(theLogger);
...
static void flowlogBeginForeignScan(ForeignScanState *node, int eflags)
{
    ...
    FlowlogFdwScanState * scan_state = (FlowlogFdwScanState *) palloc0(sizeof(FlowlogFdwScanState));
    scan_state->idx = 0;
...
static TupleTableSlot *flowlogIterateForeignScan(ForeignScanState *node)
{
    ...
    FlowlogFdwScanState *scan_state = (FlowlogFdwScanState *) node->fdw_state;
    if (scan_state->idx > 1)
        return nullptr;
    ...
    static const char *values[2][3] ={ { "11", "12", "13" },
                                       { "21", "22", "23" }  };
    HeapTuple tuple = BuildTupleFromCStrings(
			TupleDescGetAttInMetadata(node->ss.ss_currentRelation->rd_att),
			(char **)values[scan_state->idx++]);
...
static void flowlogReScanForeignScan(ForeignScanState *node)
{
    ...
    /* ----
     * FlowlogFdwScanState *scan_state =
     *   (FlowlogFdwScanState *) node->fdw_state;
     * ----
     */
                                        ...
                                        int          subplan_index)
{
    LOG_LINE(theLogger);
    LogLine(theLogger, "root->type      : %s", text_enum(root->type).c_str());
    LogLine(theLogger, "plan->operation : %s", text_enum(plan->operation).c_str());
...
{
    LOG_LINE(theLogger);
    LogLine(theLogger, "mtstate->operation : %s", text_enum(mtstate->operation).c_str());
    LogLine(theLogger, "mtstate->mt_nplans : %d", mtstate->mt_nplans);
    /*
    ...
    FlowlogFdwModifyState *modify_state =
        (FlowlogFdwModifyState *)palloc0(sizeof(FlowlogFdwModifyState));
                                                ...
                                                TupleTableSlot *planSlot)
{
    ...
    /* ----
     * FlowlogFdwModifyState *modify_state =
     *   (FlowlogFdwModifyState *) rinfo->ri_FdwState;
     * ----
     */
    ...
    LogLine(theLogger, "estate->type : %s",  text_enum(estate->type).c_str());
    LogLine(theLogger, "rinfo->type  : %s",  text_enum(rinfo->type).c_str());
    LogLine(theLogger, "slot         :\n%s", text_enum(slot).c_str());
    LogLine(theLogger, "planSlot     :\n%s", text_enum(planSlot).c_str());
...
static void flowlogExplainForeignScan(ForeignScanState *node, struct ExplainState * es)
{
    LOG_LINE(theLogger);
...
static RowMarkType flowlogGetForeignRowMarkType(RangeTblEntry *rte, LockClauseStrength strength)
{
    LOG_LINE(theLogger);
...
static List *flowlogImportForeignSchema(ImportForeignSchemaStmt *stmt, Oid serverOid)
{
    LOG_LINE(theLogger);
